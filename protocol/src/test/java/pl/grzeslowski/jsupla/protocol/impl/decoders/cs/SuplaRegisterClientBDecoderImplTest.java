...
public class SuplaRegisterClientBDecoderImplTest extends DecoderTest {
    ...
    public void shouldParseEntity() throws Exception {
        ...
        // given
        byte[] bytes = new byte[SuplaRegisterClientB.SIZE];
        int offset = 5;
        ...
        verify(primitiveDecoder).copyOfRange(bytes, offset + INT_SIZE, offset + INT_SIZE + SUPLA_ACCESSID_PWD_MAXSIZE);
        ...
        verify(primitiveDecoder).copyOfRange(bytes, offset + INT_SIZE + SUPLA_ACCESSID_PWD_MAXSIZE + SUPLA_GUID_SIZE,
                offset + INT_SIZE + SUPLA_ACCESSID_PWD_MAXSIZE + +SUPLA_GUID_SIZE + SUPLA_CLIENT_NAME_MAXSIZE);
        verify(primitiveDecoder).copyOfRange(bytes, offset + INT_SIZE + SUPLA_ACCESSID_PWD_MAXSIZE + SUPLA_GUID_SIZE + SUPLA_CLIENT_NAME_MAXSIZE,
                offset + INT_SIZE + SUPLA_ACCESSID_PWD_MAXSIZE + +SUPLA_GUID_SIZE + SUPLA_CLIENT_NAME_MAXSIZE + SUPLA_SOFTVER_MAXSIZE);
        verify(primitiveDecoder).copyOfRange(bytes, offset + INT_SIZE + SUPLA_ACCESSID_PWD_MAXSIZE
                                                            + SUPLA_GUID_SIZE + SUPLA_CLIENT_NAME_MAXSIZE + SUPLA_SOFTVER_MAXSIZE,
                offset + INT_SIZE + SUPLA_ACCESSID_PWD_MAXSIZE + SUPLA_GUID_SIZE + SUPLA_CLIENT_NAME_MAXSIZE
