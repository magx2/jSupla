...
import pl.grzeslowski.jsupla.protocol.api.decoders.ds.SuplaRegisterDeviceEDecoder;
...
public final class SuplaRegisterDeviceEDecoderImpl implements SuplaRegisterDeviceEDecoder {
    public static final SuplaRegisterDeviceEDecoderImpl INSTANCE = new SuplaRegisterDeviceEDecoderImpl(
        SuplaDeviceChannelCDecoderImpl.INSTANCE);
    private final SuplaDeviceChannelCDecoder suplaDeviceChannelCDecoder;

    @Override
    public SuplaRegisterDeviceE decode(final byte[] bytes, int offset) {
        ...
        val channels = new SuplaDeviceChannelC[channelCount];
        for (int i = 0; i < channelCount; i++) {
            ...
            channels[i] = suplaDeviceChannelCDecoder.decode(bytes, offset);
            offset += SuplaDeviceChannelC.SIZE;
        }

        return new SuplaRegisterDeviceE(
            email,
            authKey,
            guid,
            name,
